...
describe("PageParamsManager (Refactored)", () => {
  describe("shouldResetPage", () => {
    it("returns true if at least one 'shouldResetPage' param is in newParams", () => {
      expect(pageParamsManager.shouldResetPage({ days: "7" })).toBe(true);
  ...
  describe("sanitizeSearchParams", () => {
    ...
    it("removes param if it matches its default (e.g. pageSize, days)", () => {
      const defaultPageSize =
        PAGE_SIZE_OPTIONS[DEFAULT_PAGE_SIZE_INDEX].value.toString();
      const defaultDays = DAYS_FILTER_OPTIONS[DEFAULT_DAY_INDEX].value;
  ...
  describe("getParsedParams", () => {
    it("applies default values when param is absent", () => {
      ...
      expect(parsed.pageSize).toBe(
        PAGE_SIZE_OPTIONS[DEFAULT_PAGE_SIZE_INDEX].value,
      );
      expect(parsed.days).toBe(DAYS_FILTER_OPTIONS[DEFAULT_DAY_INDEX].value);

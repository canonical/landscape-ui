...
const SecurityProfileAuditPassRate: FC<SecurityProfileAuditPassRateProps> = ({
  profile,
}) => {
    ...
    <>
      <div className={classes.textContainer}>
          ...
          {passing > 0 ? (
            <Link
              to={ROUTES.instances.root({
                query: `security-profile:${profile.id}:pass`,
              })}
            >
        ...
        {failing > 0 ? (
          <Link
            to={ROUTES.instances.root({
              query: `security-profile:${profile.id}:fail`,
            })}
          >
...
export default SecurityProfileAuditPassRate;

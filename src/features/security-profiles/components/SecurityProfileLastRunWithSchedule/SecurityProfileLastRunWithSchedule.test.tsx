...
import SecurityProfileLastRunWithSchedule from "./SecurityProfileLastRunWithSchedule";
import { securityProfiles } from "@/tests/mocks/securityProfiles";
import { NO_DATA_TEXT } from "@/components/layout/NoData";
import userEvent from "@testing-library/user-event";

const [profile, profileWithRuns] = securityProfiles;

describe("SecurityProfileLastRunWithSchedule", () => {
  const user = userEvent.setup();

  it("renders no-data placeholder when runs are missing", async () => {
    renderWithProviders(
      <SecurityProfileLastRunWithSchedule profile={profile} />,
    );
  ...
  it("renders formatted last run and schedule labels", async () => {
    renderWithProviders(
      <SecurityProfileLastRunWithSchedule profile={profileWithRuns} />,
    );

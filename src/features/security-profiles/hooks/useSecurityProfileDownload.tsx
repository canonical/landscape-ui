...
type SecurityProfileDownloadMode = "audit" | "tailoring";
...
const withExtension = (
  filename: string,
  mimeType: string,
  mode: SecurityProfileDownloadMode,
) => {
  if (/\.[^./]+$/.test(filename)) {
    return filename;
  }
...
export const useSecurityProfileDownload = (
  mode: SecurityProfileDownloadMode,
) => {
  ...
  return async (path: string | null, filename?: string) => {
    ...
    try {
      const { data, headers } = await getSecurityProfileAuditDownload({
        path,
      });

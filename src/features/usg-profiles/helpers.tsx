...
export const notifyCreation = (
  ...
  notify.success({
    title: `You have successfully created ${values.title} USG profile.`,
    message: `This profile will ${phrase(notificationMessageParts)}.`,
  });
};

export const getTags = (profile: USGProfile) =>
  profile.all_computers
    ? "All instances"
    : profile.tags.join(", ") || <NoData />;

export const getTailoringFile = (profile: USGProfile) => {
...
export const getUsgSchedule = (profile: USGProfile, short = false) => {
...
export const getStatus = (profile: USGProfile) => {
  if (profile.status === "archived") {
    return { label: "Archived", icon: "status-queued-small" };
  }

  if (profile.associated_instances > USG_PROFILE_ASSOCIATED_INSTANCES_LIMIT) {
    return {
      label: (
...
export const getInitialValues = (profile: USGProfile): USGProfileFormValues => {
  const schedule = Object.fromEntries(
    profile.schedule.split(";").map((part) => part.split("=")),
  );

...
export const getDistributions = (pockets: RepositoryProfilePocket[]) => {
  ...
  pockets.forEach(({ distribution, series, ...pocket }) => {
    ...
    const existingSeries = existingDistribution?.series.find(
      ({ name }) => name === series.name,
    );

    const pocketWithDistributionAndSeries = { ...pocket, distribution, series };
    const seriesWithPocket = {
      ...series,
      pockets: [pocketWithDistributionAndSeries],
    };
    const distributionWithSeries = {
      ...distribution,
      series: [seriesWithPocket],
    };

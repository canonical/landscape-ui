...
import type {
  LocalServiceListLocalsError,
  LocalServiceListLocalsResponse,
  Local,
...
export const useGetLocalRepositories = (search?: string) => {
  ...
  const { data, isPending } = useQuery<
    Local[],
    AxiosError<LocalServiceListLocalsError>
  >({
    ...
    queryFn: async () => {
      ...
      do {
        const response =
          await authFetchDebArchive.get<LocalServiceListLocalsResponse>(
            "locals",
            {
              params: {

...
describe("ViewLocalRepositorySidePanel", () => {
  ...
  it("renders loading state when fetching repository", () => {
    renderWithProviders(<ViewLocalRepositorySidePanel />, undefined, "?sidePath=view&name=aaaa-bbbb-cccc");
  ...
  it("defaults to details tab and changes to packages tab when clicked", async () => {
    ...
    expect(screen.getByText(/general details/i)).toHaveAttribute("aria-selected", "true");

    await user.click(screen.getByText(/packages/i));

    expect(screen.getByText(/packages/i)).toHaveAttribute("aria-selected", "true");
    expect(screen.getByText(/general details/i)).not.toHaveAttribute("aria-selected", "true");
  });
});

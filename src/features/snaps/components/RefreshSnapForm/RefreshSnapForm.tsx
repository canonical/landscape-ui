...
const RefreshSnapForm: FC<RefreshSnapFormProps> = ({ installedSnaps }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        closeSidePanel();
        notify.success({
          message: `You queued ${pluralize(installedSnaps.length, ["snap"], "exact")} to be refreshed.`,
        });
      } catch (error) {
        debug(error);
      }
...
export default RefreshSnapForm;

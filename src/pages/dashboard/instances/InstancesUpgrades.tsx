...
import classes from "./InstancesUpgrades.module.scss";
...
interface InstancesUpgradesProps {
  selectedInstances: Instance[];
}

const InstancesUpgrades: FC<InstancesUpgradesProps> = ({
  selectedInstances,
}) => {
  const [securityIssuesLimit, setSecurityIssuesLimit] = useState(2);
  const [affectedInstancesLimit, setAffectedInstancesLimit] = useState(5);
  ...
  const {
    ...
    queryKey: [
      "mockedUsns",
      {
        ...
        offset: 0,
        query: selectedInstances.map(({ id }) => id),
      },
    ],
    queryFn: () =>
      GET_USNS({
    ...
    enabled: !!selectedInstances.length,
  });
  ...
  const securityIssueColumns = useMemo<Column<(typeof mockUsns)[number]>[]>(
    ...
    [selectedInstances.length, expandedUsn, expandedPackages.length],
  );
  ...
  const affectedInstances = useMemo<Instance[]>(
    () => selectedInstances.slice(0, affectedInstancesLimit),
    [selectedInstances, affectedInstancesLimit],
  );
    ...
    <Form onSubmit={handleSubmit}>
      ...
      <div>
        ...
        <ExpandableTableFooter
          ...
          itemName="instance"
          limit={affectedInstancesLimit}
          onClick={() => {
            setAffectedInstancesLimit((prevState) => prevState + 5);
          }}
...
export default InstancesUpgrades;

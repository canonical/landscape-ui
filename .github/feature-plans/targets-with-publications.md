...
`useGetPublicationTargets` unconditionally calls the local `useGetPublications()` (which paginates through *all* publications) and client-side joins the result into every list row. The publications payload grows linearly with account activity and is O(total publications) regardless of how many targets are displayed.

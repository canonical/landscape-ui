...
export var Modal = _ref => {
  ...
  var getVisibleFocusableElements = () => {
    if (!modalRef.current) {
      return [];
    }
    return Array.from(modalRef.current.querySelectorAll(focusableElementSelectors)).filter(isElementVisible);
  };
  var handleTabKey = event => {
    var focusableModalElements = getVisibleFocusableElements();
  ...
  useEffect(() => {
    if (focusRef !== null && focusRef !== void 0 && focusRef.current) {
      ...
      if (isElementVisible(closeButtonRef.current)) {
        ...
        var firstFocusable = getVisibleFocusableElements()[0];
        if (firstFocusable) {

...
`src/main.tsx` also imports `@canonical/styles/fonts`, which provides the `Ubuntu Sans` font
family used by Pragma's typography tokens. This is required even while vanilla-framework remains
